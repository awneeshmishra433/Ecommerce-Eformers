...
Setup
1. Copy `.env.example` to `.env` and fill in the required values (e.g. `MANGODB_URI`, `JWT_SECRET`, Cloudinary keys).
2. Run `npm install` then `npm start` or `npm run server` to start the development server.
