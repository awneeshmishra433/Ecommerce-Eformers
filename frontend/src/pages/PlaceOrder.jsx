...
const PlaceOrder = () => {
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          ...
          if (data.success) {
            setCartItems({});            removeCoupon();            navigate("/orders");
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
        ...
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            ...
            if (itemInfo) {
              ...
              // compute unit price for the chosen size
              const parseSizeNumeric = (s) => {
                if (!s) return null;
                const m = String(s).match(/[0-9]+(?:\.[0-9]+)?/);
                return m ? parseFloat(m[0]) : null;
              };
              const numeric = parseSizeNumeric(itemInfo.size);
              ...
              itemInfo.price = Number(unitPrice?.toFixed(2));
      ...
      const finalAmount = Math.round((subtotal - discountAmount + delivery_fee) * 100) / 100;
