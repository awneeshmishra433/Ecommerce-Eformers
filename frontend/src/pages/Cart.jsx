...
const Cart = () => {
  const { products, currency, cartItems, updateQuantity, navigate, token, applyCoupon, removeCoupon, couponCode, couponDiscount } =
    useContext(ShopContext);
  const [cartData, setCartData] = useState([]);
  const [proceedToPayment, setProceedToPayment] = useState(false);
  const [promoCode, setPromoCode] = useState("");

  // Note: Promo code definitions / validation live in ShopContext (applyCoupon)


  useEffect(() => {
    // Allow viewing cart without login. Checkout will require authentication.
  }, []);
      ...
      <div>
        {cartData.map((item, index) => {
          ...
          if (!productData) return null;
            // compute per-size price (use pricePerKg if available)
            ...
            const unitNumeric = parseSizeNumeric(item.size);

            let unitPrice = productData?.price || 0;

            if (productData?.pricePerKg && unitNumeric) {
              unitPrice = Number((productData.pricePerKg * unitNumeric).toFixed(2));
            } else {
              const sizesNumeric = (productData?.sizes || [])
                .map(parseSizeNumeric)
                .filter(Boolean)
                .sort((a,b)=>a-b);
              const smallest = sizesNumeric && sizesNumeric.length > 0 ? sizesNumeric[0] : null;
              if (smallest && productData?.price && unitNumeric) {
                const pricePerUnit = productData.price / smallest;
                unitPrice = Number((pricePerUnit * unitNumeric).toFixed(2));
              }
            }
        ...
        <div className="flex justify-end my-20">
          <div className="w-full sm:w-[450px]">
            ...
            <div className="mt-6 mb-4">
              <p className="text-gray-700 text-sm mb-2">If you have a promo code, enter it here</p>
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder="Enter promo code"
                  className="flex-1 px-4 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-gray-500"
                  value={promoCode}
              ...
              {couponDiscount > 0 && (
                <div className="flex items-center gap-3 mt-2">
                  <p className="text-green-600 text-sm">Promo code applied: {couponCode} — {couponDiscount}% off</p>
                  <button
